    ...
    return (
      <div className="space-y-1.5 w-full">
        ...
        <textarea
          ...
          rows={rows}
          disabled={disabled}
          className={`w-full p-3.5 bg-white/80 border ${
            error
              ? 'border-rose-400 ring-4 ring-rose-500/10 focus:border-rose-500'
              : 'border-slate-200/90 hover:bg-white hover:border-slate-300 focus:border-[#3B82F6] focus:ring-4 focus:ring-[#3B82F6]/15 focus:bg-white'
          } rounded-2xl text-xs sm:text-sm font-semibold text-slate-800 placeholder:text-slate-400 placeholder:font-normal outline-none resize-none transition-all shadow-xs ${
            disabled
