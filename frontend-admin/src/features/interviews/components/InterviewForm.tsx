...
export default function InterviewForm({
  ...
  const [isLoadingInterview, setIsLoadingInterview] = useState<boolean>(mode === 'edit');
  ...
  // Live Interviewer Conflict Check Effect
  useEffect(() => {
    if (!selectedInterviewerId || !dateVal || !startTimeVal || !endTimeVal) {
    ...
    const timer = setTimeout(() => {
      interviewsApi
        .checkConflict({
          ...
          endTime: endTimeVal,
          excludeInterviewId: mode === 'edit' ? interviewId : undefined,
        })
        .then((res) => {
  ...
  }, [selectedInterviewerId, dateVal, startTimeVal, endTimeVal, mode, interviewId]);
  ...
  useEffect(() => {
    ...
    const populateInterviewData = (data: InterviewItem) => {
      setSelectedAppId(data.applicationId);

      const cand = data.candidateId;
      const candName = typeof cand === 'object' ? cand?.fullName || cand?.name : 'Ứng viên';
      const jobObj = data.jobDescriptionId;
      const jobTitle = typeof jobObj === 'object' ? jobObj?.title : 'Vị trí tuyển dụng';
      const deptName =
        typeof jobObj === 'object' && typeof jobObj?.departmentId === 'object'
          ? jobObj.departmentId.name
          : 'Phòng ban';

      setCandidateSearchQuery(`${candName} — ${jobTitle}`);

      const interviewerId =
        typeof data.interviewerId === 'object' ? data.interviewerId._id : data.interviewerId || '';
      ...
      setStartTimeVal(data.startTime || '14:00');
      setEndTimeVal(data.endTime || '15:30');
      setLocationTypeVal(data.locationType || LocationType.ONLINE);
      setOffsiteLocationVal(data.offsiteLocation || '');
      setNotesVal(data.notes || '');
      setStatusVal(data.status || InterviewStatus.SCHEDULED);

      setIsLoadingInterview(false);
    };

    if (initialInterview) {
      populateInterviewData(initialInterview);
    } else if (interviewId) {
      setIsLoadingInterview(true);
      interviewsApi
        .getInterviewById(interviewId)
        .then((data) => {
          if (data) populateInterviewData(data);
        })
        .catch((err) => {
          console.error('Lỗi khi tải thông tin lịch phỏng vấn:', err);
          setErrorMsg('Không thể tải thông tin buổi phỏng vấn này.');
        })
        .finally(() => {
          setIsLoadingInterview(false);
        });
    }
  }, [mode, interviewId, initialInterview]);

  // Handle candidate selection change & populate department staff options
  useEffect(() => {
    if (!selectedAppId || !candidateOptions.length) return;
    const found = candidateOptions.find((c) => c.applicationId === selectedAppId);
    if (found) {
      setSelectedCandidate(found);
      if (found.departmentStaff && found.departmentStaff.length > 0) {
        setInterviewerOptions(found.departmentStaff);
      }
    }
  }, [selectedAppId, candidateOptions]);

  // Global Unsaved Changes Interception
  useEffect(() => {
    ...
    try {
      ...
      setErrorMsg(
        err?.response?.data?.message ||
          err?.message ||
          `${mode === 'create' ? 'Tạo' : 'Cập nhật'} lịch phỏng vấn thất bại. Vui lòng thử lại!`
      );
    } finally {
  ...
  const getSelectedInterviewerLabel = () => {
    if (!selectedAppId && mode === 'create') return '-- Vui lòng chọn ứng viên trước --';
  ...
  if (isLoadingInterview) {
    return (
      <div className="py-20 text-center space-y-3">
        ...
        <p className="text-xs font-semibold text-slate-500">Đang tải thông tin buổi phỏng vấn...</p>
      </div>
    );
  }
      ...
      <div className="bg-white border border-slate-200/80 rounded-3xl shadow-sm overflow-hidden">
        ...
        <div className="p-6 sm:p-8 border-b border-slate-100 bg-gradient-to-r from-slate-50/80 via-white to-indigo-50/40 flex items-center justify-between">
            ...
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <span className="text-[11px] font-bold text-indigo-600 uppercase tracking-wider">
                  {mode === 'create' ? 'Lên lịch phỏng vấn mới' : 'Chỉnh sửa thông tin phỏng vấn'}
                </span>
              </div>
              <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight">
                {mode === 'create' ? 'Tạo lịch phỏng vấn' : 'Chỉnh sửa lịch phỏng vấn'}
              </h1>
              <p className="text-xs text-slate-500 font-medium mt-0.5">
                {mode === 'create'
                  ? 'Lên lịch và gửi thông báo tự động đến ứng viên & interviewer'
                  : 'Cập nhật ngày, giờ, hình thức phỏng vấn và người phụ trách'}
              </p>
            </div>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-3 shrink-0">
            <span className="px-3.5 py-1.5 rounded-2xl bg-indigo-50 border border-indigo-100 text-indigo-700 text-xs font-extrabold flex items-center gap-1.5">
              {mode === 'create' ? <Sparkles size={14} /> : <Edit3 size={14} />}
              <span>{mode === 'create' ? 'Thiết lập lịch phỏng vấn' : 'Cập nhật lịch phỏng vấn'}</span>
            </span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-7">
          ...
          <div className="space-y-2 relative" ref={candidateComboboxRef}>
            ...
            {mode === 'edit' ? (
              <div className="p-4 bg-gradient-to-r from-slate-50 via-indigo-50/20 to-emerald-50/20 border border-slate-200/80 rounded-2xl flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <UserIcon size={16} className="text-indigo-600" />
                  <span className="text-sm font-extrabold text-slate-900">{candidateSearchQuery}</span>
                </div>
            ...
            {selectedAppId && selectedCandidate && candidateSearchQuery === selectedCandidate.label && mode === 'create' && (
          ...
          <div className="space-y-2 relative" ref={interviewerDropdownRef}>
            ...
            <div className="relative">
              <button
                type="button"
                disabled={!selectedAppId && mode === 'create'}
                onClick={() => {
                  if (selectedAppId || mode === 'edit') {
          ...
          {conflictInfo && (
            <div className="p-4 bg-amber-50 border border-amber-300 text-amber-900 rounded-2xl text-xs font-semibold flex items-start gap-3 shadow-xs animate-in fade-in">
              <AlertCircle size={18} className="shrink-0 text-amber-600 mt-0.5" />
              <div>
                <span className="font-bold text-amber-950 block">⚠️ Cảnh báo trùng lịch phỏng vấn!</span>
                <span>
                  Người phỏng vấn được chọn đã có một buổi phỏng vấn khác với ứng viên{' '}
                  <strong className="text-slate-900">&ldquo;{conflictInfo.candidateName}&rdquo;</strong> vào khung giờ{' '}
                  <strong className="text-slate-900">{conflictInfo.timeSlot}</strong> ngày{' '}
                  <strong className="text-slate-900">{conflictInfo.dateFormatted}</strong>. Vui lòng chọn khung giờ hoặc đổi Người phỏng vấn khác.
                </span>
              </div>
            </div>
          )}

          {/* 4. Hình thức & Trạng thái */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-700 uppercase tracking-wider block">
                Hình thức phỏng vấn
            ...
            {mode === 'edit' && (
              <div className="space-y-2">
                ...
                <CustomSelect
                  ...
                  value={statusVal}
                  onChange={(val) => {
                    setStatusVal(val as InterviewStatus);
                    setIsDirty(true);
                  }}
                />
              </div>
          ...
          {locationTypeVal === LocationType.ONLINE && (
            <div className="p-4 bg-indigo-50/60 border border-indigo-100 rounded-2xl flex items-start gap-3">
              ...
              />
              <label htmlFor="autoMeetForm" className="cursor-pointer select-none">
          ...
          <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <Calendar size={16} />
                  <span>{mode === 'create' ? 'Tạo lịch & gửi thông báo' : 'Lưu thay đổi'}</span>
                </>
              )}
            </button>
          </div>
