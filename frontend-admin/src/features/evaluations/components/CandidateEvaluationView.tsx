...
export default function CandidateEvaluationView({
  ...
  const handleSubmitEvaluation = async () => {
    ...
    if (unrated.length > 0) {
      if (!window.confirm(`Vẫn còn ${unrated.length} tiêu chí chưa chấm điểm. Bạn có muốn tiếp tục nộp đánh giá?`)) {
        return;
      }
    }
  ...
  if (fetchError || !interview) {
    return (
      <div className="py-16 text-center max-w-md mx-auto space-y-4 bg-white/40 border border-white/60 rounded-3xl p-8 shadow-xl">
        <div className="p-3 bg-rose-100 text-rose-600 rounded-2xl w-fit mx-auto font-bold text-xs">
  ...
  return (
    <div className="relative w-full pb-8 space-y-4 animate-fade-in">
      ...
      {toast && (
        ...
        >
          {toast.type === "success" && <CheckCircle2 size={18} className="shrink-0 text-white" />}
          {toast.type === "warning" && <AlertCircle size={18} className="shrink-0 text-white" />}
          {toast.type === "error" && <AlertCircle size={18} className="shrink-0 text-white" />}
      ...
      <div className="sticky top-0 z-30 space-y-3 pb-2 pt-0 transition-all">
        <EvaluationHeader
